...
public class ExecThreadPoolExecutor {
    ...
    public void outApiThreadPoolExecutorLogger() {
        ...
        buffer.append(" 系统当前线程数：" + JMeterThreadUtil.activeCount()).append("\n");
    ...
    public void removeQueue(String reportId) {
        ...
        workerQueue.forEach(item -> {
            SystemExecTask task = (SystemExecTask) item;
    ...
    public boolean check(String reportId) {
        ...
        // 检查等待队列
        BlockingQueue workerQueue = threadPool.getQueue();
        return workerQueue.stream().filter(task -> StringUtils.equals(((SystemExecTask) task).getRequest().getReportId(), reportId)).count() > 0;
    }
    ...
    public boolean checkPlanReport(String planReportId) {
        ...
        // 检查等待队列
        BlockingQueue workerQueue = threadPool.getQueue();
        return workerQueue.stream().filter(task -> StringUtils.equals(((SystemExecTask) task).getRequest().getTestPlanReportId(), planReportId)).count() > 0;
    }
